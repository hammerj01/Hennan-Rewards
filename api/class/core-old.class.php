<?php
	...
	class core extends mailer {
		...
		public function temp_registration($param) {
			...
			$global_temp_registration = process_qry("CALL global_temp_registration(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", array($data->email, $data->password, $data->fname, $data->mname, $data->lname, $data->mobileNum, $data->landlineNum, $data->dateOfBirth, $data->gender, $data->address1, $data->address2, $data->city, $data->province, $data->region, $data->zip, $data->civilStatus, $data->occupation, $data->regtype, $data->platform, $activation, $data->qrCard, NULL, $data->locID, $data->serverName, $data->nationality), $logs);
		...
		public function activation($param){
			...
			$logs->write_logs("Error - Activation", $file_name, array(array("response"=>"Error", "description"=>"Unable to complete activation process.", "data"=>array($param))));
		...
		public function resend_activation($param) {
			...
			if ($step_1[0]['count'] > 0) {
				...
				$step_2 = process_qry("SELECT COUNT(*) AS `count`, `fname` FROM `temp_memberstable` WHERE `email` = ? LIMIT 1", array($data->email), $logs);
			...
			return json_encode(array(array("response"=>"Error", "description"=>"Unable to complete resending of activation process.")));
		}
		...
		public function reset_password($param) {
			...
			if ($step_1[0]['count'] > 0) {
				...
				$this->emailer(array("recipient_name" => $step_1[0]['fname'], "email" => $data->email, "password" => $password, "subject" => $subject, "activation" => NULL, "type" => "reset_password", "emailFromFB" => NULL));
				$logs->write_logs("Success - Reset Password for email", $file_name, array(array("response"=>"Success", "data"=>array($param))));
				return json_encode(array(array("response"=>"Success", "data"=>array(array("description"=>"Please check your email for the Reset Password.")))));				
			} else {
		...
		public function social_login($param) {
			...
			$logs->write_logs("facebook data - Facebook Registration", $file_name, array(array("response"=>"Success", "data"=>array(array($data->email, $data->pid, $data->deviceID, $data->platform, $data->fname, $data->mname, $data->lname, $data->dateOfBirth, $data->gender, $data->image, $activation, $data->regtype, $data->memberCode, $data->locID, $memberID, $qrApp,  $data->password, $data->brandID, $data->mobileNum)))));

			$step_5 = process_qry("CALL global_social_login(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", array($data->email, $data->pid, $data->deviceID, $data->platform, $data->fname, $data->mname, $data->lname, $data->dateOfBirth, $data->gender, $data->image, $activation, $data->regtype, $data->memberCode, $data->locID, $memberID, $qrApp, $data->password, $data->brandID, $data->mobileNum), $logs);
			...
			if ($step_1[0]['count']  > 0 || $notActivated[0]['cnt'] > 0 ) {
				...
				if ($step_1[0]['lockStatus'] == "true") {
					...
					if ($step_1[0]['status'] == "inactive") {
						...
						return json_encode(array(array("response"=>"Error", "description"=>"Account is inactive.")));
					} else {

						if (isset($data->email) || $data->email != NULL){	
							$step_2 = process_qry("SELECT COUNT(*) AS `count`, `memberID` FROM `memberstable` WHERE `email` = ? AND `password` = PASSWORD(?) LIMIT 1", array($data->email, $data->password), $logs);
						}
		...
		public function complete_data($param) {
			...
			$step_1 = process_qry("SELECT COUNT(*) AS `count` FROM `memberstable` WHERE `memberID` = ? LIMIT 1", array($data->memberID), $logs);
			...
			if ($step_1[0]['count'] > 0) {
				...
				if ($step_3[0]['count'] > 0) {
					$output = process_qry("SELECT `memberID`, `fname`, `mname`, `lname`, `email`, `mobileNum`, `landlineNum`, `dateReg`, `qrApp`, `qrCard`, `occupation`, `civilStatus`, `dateOfBirth`, `address1`, `address2`, `city`, `province`, `zip`, `gender`, `image`, `profileStatus`, `dateReg`, `totalPoints`, `accumulatedPoints`, `lastTransaction`,`expiration`, (SELECT `locName` FROM `earntable` WHERE `memberID` = ? ORDER BY `dateAdded` DESC LIMIT 1) AS `lastVisited`, `levelID`, `levelName`, ? AS `levelMax` FROM `memberstable` WHERE `memberID`= ? LIMIT 1", array($data->memberID, $level[0]['max'], $data->memberID), $logs);
		...
		public function request_card($param){
			...
			if($getmember[0]['count'] > 0){
				...
				$step_1 = process_qry("SELECT IF(MAX(`id`) IS NULL, 0, MAX(`id`)) AS `entry` FROM `request_card_queue_arc` LIMIT 1", array());
				$x = 0;
				$requestID = NULL;

				while ($x < 1) {
					$requestID = substr("REQ" . randomizer(2) . DATE("H") . $step_1[0]['entry'] . DATE("s") . randomizer(6), 0, 16);
					$check_requestID = process_qry("SELECT count(*) AS `count` FROM `request_card_queue_arc` WHERE `requestID` = ? LIMIT 1", array($requestID), $logs);
					$x = ( $check_requestID[0]['count'] < 1 ) ? 1 : 0;
				}
				$mdate = date('Y-m-d', strtotime(date('Y-m-d')));
				$INSERT_requestCard = process_qry("INSERT INTO request_card(`requestID`,`memberID`,`fname`,`lname`,`house_no`,`street_no`,`barangay`,`city`,`mobileNum`,`telNum`,`status`,`dateAdded`,`dateModified`) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? , ?, ?)",array($requestID,$data->memberID,$data->fname,$data->lname,$data->house_no,$data->street_no,$data->barangay,$data->city,$data->mobileNum,$data->telNum,"inactive",$mdate,$mdate), $logs);
				$logs->write_logs("Success - Request Card", $file_name, array(array("response"=>"Success", "data"=>array($param))));
		...
		public function emailer($param) {
			...
			if(($data->type == 'temp_registration') || $data->type == 'resend_activation') {
				$message .= "<p>Please activate your account by clicking the button below.</p><a href='" . DOMAIN . "/activation.php?email=" . $data->email . "&activation=" . $data->activation . "'><div class='activate'>Activate</div></a><p>Once successful, you may now use " . MERCHANT . " to earn points and redeem rewards!</p>";
		...
		// === start mailer to merhant
		public function emailer_merchant($param) {
			global $logs;
			global $protocol;
			$data = (object) $param;
			$file_name = 'core.class.php';
			$message = "<!DOCTYPE html><html><head><title></title><style type='text/css'>body, html{margin: 0;padding: 0 5%;font-family: 'Verdana';font-size: 14px;} img{width: 100%;} header{width: 100%;} .container{background-color: #f5f5f5;padding: 0 0 100px 0;} .emailBody{margin-top: 5%;padding: 0 5%;} .emailBody p{text-align: left;margin-top: 20px;line-height: 1.5} #regDetails p{margin: 0 0 0 20px;} #regDetails p label{font-weight: bold;} .activate{background-color: #35b7e8;padding: 20px;border-radius: 5px;width: 20%;margin: 5% auto;color: #fff;text-transform: uppercase;text-align: center;font-size: 18px;} a {text-decoration: none;}</style></head><body><div class='container'><header><img src='" . DOMAIN . "/assets/images/activation/email_header.png'></header>";
			if($data->type == 'request_card_notify_to_merchant'){
				$message .= "<div class='emailBody'><h2> Admin, </h2> <p> Please be inform that " . $data->email . " has requested for a physical membership cad. Below are the details of the of the customer: </p> <br /> Name: " . $data->full_name ." <br /> Address: " . $data->address . " <br /> Contact Number: " . $data->mobileNum ." </div><br /><br />Thank you! <br /><br /><p> Regards, <br /> Henann Group of Resorts Support Department <p><center>**This is a system-generated message. Please do not reply to this email.**</center></p></div></body></html>";
			}
			// elseif($data->type == 'request_card_notify_to_merchant'){

			// }

			$sent = $this->send_mail(array('recipient' => $data->email, 'recipient_name' => $data->recipient_name, 'subject' => $data->subject, 'message' =>  $message, 'attachment' => NULL));
			
		} // === end emailer to merchant
				...
				if(count($qry_card_voucher) > 0 ){	
					for ($i=0; $i < count($qry_card_voucher); $i++) { 
						for ($x=0; $x < $qry_card_voucher[$i]['quantity']; $x++) { 
							...
							$insert_voucher =process_qry("INSERT INTO `earnvouchertable` (`voucherID`, `memberID`, `email`, `name`, `startDate`, `endDate`, `type`, `action`, `status`, `dateAdded`, `dateModified`, `image`) SELECT `voucherID`, ? as memberID , ? AS `email`, `name`, NOW() as `startDate` , DATE_ADD(NOW(), INTERVAL 1 YEAR) AS `endDate`, `type`, `action`, `status`, NOW() AS `dateAdded`, NOW() AS `dateModified`, `image` FROM `vouchertable` WHERE   `type` = 'registration' AND  `cardType` = ? AND  `status` = 'active' AND  `cardVersion` = ? and `voucherID` = ?",array($memberID,$qry_member[0]['email'],$qry_card_voucher[0]['cardType'],$qry_card_voucher[0]['cardVersion'], $qry_card_voucher[$i]['voucherID']), $logs);

						}
					}
				}	
				// === birthday
				$qry_card_voucher_birthday = process_qry("SELECT count(*) as `cnt`, `quantity`,`cardType`,`cardVersion`  FROM `vouchertable` WHERE   `type` = 'birthday' AND  `status` = 'active' AND  `cardType` = 'trident'",array(), $logs);
		...
		public function my_voucher_list($param){
			...
			$step_1 = process_qry("SELECT COUNT(`id`) AS `count`, `dateOfBirth`, `lastTransaction`, `profileStatus`, `email`,`memberID` FROM `memberstable` WHERE `memberID` = ? AND `status` = 'active' LIMIT 1", array($data->memberID), $logs);
			

			if ($step_1[0]['count'] > 0) {
				$step_2 = process_qry("UPDATE `vouchertable` SET `status` = 'expired' WHERE `status` = 'active' AND DATE_FORMAT(NOW(), '%Y-%m-%d') > DATE_FORMAT(`endDate`, '%Y-%m-%d')", array(), $logs);
				$step_3 = process_qry("UPDATE `vouchertable` a SET a.`remaining` = (SELECT COUNT(*) FROM `redeemvouchertable` WHERE `voucherID` = a.`voucherID`) WHERE a.`status` = 'active' AND DATE_FORMAT(NOW(), '%Y-%m-%d') >= DATE_FORMAT(a.`startDate`, '%Y-%m-%d') AND DATE_FORMAT(a.`endDate`, '%Y-%m-%d') >= DATE_FORMAT(NOW(), '%Y-%m-%d') AND `type` = 'flash'", array(), $logs);
				$step_3 = process_qry("UPDATE `vouchertable` SET `status` = 'expired' WHERE `status` = 'active' AND `remaining` >= `quantity` AND `type` = 'flash'", array(), $logs);
				$step_4 = process_qry("UPDATE `earnvouchertable` SET `status` = 'expired' WHERE `status` = 'active' AND `memberID` = ? AND `voucherID` IN (SELECT `voucherID` FROM `vouchertable` WHERE `status` = 'expired')", array($data->memberID), $logs);
				$step_5 = process_qry("SELECT `voucherID`, `quantity`, (SELECT COUNT(*) FROM `earnvouchertable` WHERE `memberID` = ? AND `voucherID` = `vouchertable`.`voucherID` LIMIT 1) AS `ev_count`, (SELECT COUNT(*) FROM `redeemvouchertable` WHERE `memberID` = ? AND `voucherID` = `vouchertable`.`voucherID` LIMIT 1) AS `rv_count` FROM `vouchertable` WHERE `status` = 'active'", array($data->memberID, $data->memberID), $logs);

				$allowed_months = process_qry("select period_diff(extract(year_month from curdate()),extract(year_month from `expiration`)) as remaining from `memberstable` where `memberID` = ?",array($data->memberID), $logs);
				
				if( $allowed_months[0]['remaining'] > 3 ){
					return json_encode(array(array("response"=>"Error", "description"=> "expired account " . $step_1[0]['memberID'] )));
				}
					...
					for ($i = 0; $i < count($step_5); $i++) {
						...
						if ($v_count > 0) {
							for ($ii = 0; $ii < $v_count; $ii++) {
								$step_6 = process_qry("INSERT INTO `earnvouchertable` (`voucherID`, `memberID`, `email`, `name`, `startDate`, `endDate`, `type`, `action`, `status`, `dateAdded`, `dateModified`, `image`) SELECT `voucherID`, ? AS `memberID`, ? AS `email`, `name`, `startDate`, `endDate`, `type`, `action`, `status`, NOW() AS `dateAdded`, NOW() AS `dateModified`, `image` FROM `vouchertable` WHERE  `status` = 'active' AND `voucherID` = ?", array($data->memberID, $step_1[0]['email'], $step_5[$i]['voucherID']), $logs);
							}
						}
					}
				}
				...
				if (DATE('m', strtotime($step_1[0]['dateOfBirth'])) == DATE('m') && $step_1[0]['dateOfBirth'] != NULL ) {
					...
					if (($step_1[0]['lastTransaction'] == NULL) || ($step_1[0]['lastTransaction'] == "")) {

						if ($step_1[0]['profileStatus'] == 'complete' || $step_1[0]['profileStatus'] == 'incomplete') {
							$qry = "SELECT a.*, b.`description`, b.`terms`, COUNT(*) AS `count` FROM `earnvouchertable` a, `vouchertable` b WHERE a.`memberID` = ? AND DATE_FORMAT(NOW(), '%Y-%m-%d') >= DATE_FORMAT(a.`startDate`, '%Y-%m-%d') AND DATE_FORMAT(date_add(a.`endDate`, INTERVAL 3 MONTH), '%Y-%m-%d') >= DATE_FORMAT(NOW(), '%Y-%m-%d') AND a.`voucherID` = b.`voucherID` AND a.`status` = 'active' AND b.`status` = 'active' AND a.`type` != 'wemissyou' GROUP BY b.`voucherID`";
						} else {
							$qry = "SELECT a.*, b.`description`, b.`terms`, COUNT(*) AS `count` FROM `earnvouchertable` a, `vouchertable` b WHERE a.`memberID` = ? AND DATE_FORMAT(NOW(), '%Y-%m-%d') >= DATE_FORMAT(a.`startDate`, '%Y-%m-%d') AND DATE_FORMAT(date_add(a.`endDate`, INTERVAL 3 MONTH), '%Y-%m-%d') >= DATE_FORMAT(NOW(), '%Y-%m-%d') AND a.`voucherID` = b.`voucherID` AND a.`status` = 'active' AND b.`status` = 'active' AND a.`type` NOT IN ('wemissyou', 'registration') GROUP BY b.`voucherID`";
						}
					} else {
						if ($step_1[0]['profileStatus'] == 'complete' || $step_1[0]['profileStatus'] == 'incomplete') {
							if (DATE('m', strtotime($step_1[0]['dateOfBirth'])) == DATE('m') && $step_1[0]['dateOfBirth'] != NULL ) {

								$qry = "SELECT a.*, b.`description`, b.`terms`, COUNT(*) AS `count` FROM `earnvouchertable` a, `vouchertable` b WHERE a.`memberID` = ? AND DATE_FORMAT(NOW(), '%Y-%m-%d') >= DATE_FORMAT(a.`startDate`, '%Y-%m-%d') AND DATE_FORMAT(date_add(a.`endDate`, INTERVAL 3 MONTH), '%Y-%m-%d') >= DATE_FORMAT(NOW(), '%Y-%m-%d') AND a.`voucherID` = b.`voucherID` AND a.`status` = 'active' AND b.`status` = 'active' AND b.`voucherID` NOT IN (SELECT `voucherID` FROM `vouchertable` WHERE `status` = 'active' AND `type` = 'wemissyou' AND (SELECT DATEDIFF(NOW(), @v_lastTransaction)) < CAST(`wmyperiod` AS UNSIGNED)) GROUP BY b.`voucherID`";
							}else{
								$qry = "SELECT a.*, b.`description`, b.`terms`, COUNT(*) AS `count` FROM `earnvouchertable` a, `vouchertable` b WHERE a.`memberID` = ? AND DATE_FORMAT(NOW(), '%Y-%m-%d') >= DATE_FORMAT(a.`startDate`, '%Y-%m-%d') AND DATE_FORMAT(date_add(a.`endDate`, INTERVAL 3 MONTH), '%Y-%m-%d') >= DATE_FORMAT(NOW(), '%Y-%m-%d') AND a.`voucherID` = b.`voucherID` AND a.`status` = 'active' AND b.`status` = 'active' AND b.`voucherID` NOT IN (SELECT `voucherID` FROM `vouchertable` WHERE `status` = 'active' AND `type` = 'wemissyou' AND (SELECT DATEDIFF(NOW(), @v_lastTransaction)) < CAST(`wmyperiod` AS UNSIGNED)) AND a.`type` != 'birthday' GROUP BY b.`voucherID`";
							}


						} else {
							$qry = "SELECT a.*, b.`description`, b.`terms`, COUNT(*) AS `count` FROM `earnvouchertable` a, `vouchertable` b WHERE a.`memberID` = ? AND DATE_FORMAT(NOW(), '%Y-%m-%d') >= DATE_FORMAT(a.`startDate`, '%Y-%m-%d') AND DATE_FORMAT(date_add(a.`endDate`, INTERVAL 3 MONTH), '%Y-%m-%d') >= DATE_FORMAT(NOW(), '%Y-%m-%d') AND a.`voucherID` = b.`voucherID` AND a.`status` = 'active' AND b.`status` = 'active' AND b.`voucherID` NOT IN (SELECT `voucherID` FROM `vouchertable` WHERE `status` = 'active' AND `type` = 'wemissyou' AND (SELECT DATEDIFF(NOW(), @v_lastTransaction)) < CAST(`wmyperiod` AS UNSIGNED)) AND a.`type` != 'registration' GROUP BY b.`voucherID`";
						}
					}
				} else {
					...
					if (($step_1[0]['lastTransaction'] == NULL) || ($step_1[0]['lastTransaction'] == "")) {
						if ($step_1[0]['profileStatus'] == 'complete' || $step_1[0]['profileStatus'] == 'incomplete') {
							...
							$qry = "SELECT a.*, b.`description`, b.`terms`, COUNT(*) AS `count` FROM `earnvouchertable` a, `vouchertable` b WHERE a.`memberID` = ? AND DATE_FORMAT(NOW(), '%Y-%m-%d') >= DATE_FORMAT(a.`startDate`, '%Y-%m-%d') AND DATE_FORMAT(date_add(a.`endDate`, INTERVAL 3 MONTH), '%Y-%m-%d') >= DATE_FORMAT(NOW(), '%Y-%m-%d') AND a.`voucherID` = b.`voucherID` AND a.`status` = 'active' AND b.`status` = 'active' AND a.`type` NOT IN ('birthday', 'wemissyou') GROUP BY b.`voucherID`";
						} else {
							$qry = "SELECT a.*, b.`description`, b.`terms`, COUNT(*) AS `count` FROM `earnvouchertable` a, `vouchertable` b WHERE a.`memberID` = ? AND DATE_FORMAT(NOW(), '%Y-%m-%d') >= DATE_FORMAT(a.`startDate`, '%Y-%m-%d') AND DATE_FORMAT(date_add(a.`endDate`, INTERVAL 3 MONTH), '%Y-%m-%d') >= DATE_FORMAT(NOW(), '%Y-%m-%d') AND a.`voucherID` = b.`voucherID` AND a.`status` = 'active' AND b.`status` = 'active' AND a.`type` NOT IN ('birthday', 'wemissyou', 'registration') GROUP BY b.`voucherID`";
						}
					} else {

						if ($step_1[0]['profileStatus'] == 'complete' || $step_1[0]['profileStatus'] == 'incomplete') {

							$qry = "SELECT a.*, b.`description`, b.`terms`, COUNT(*) AS `count` FROM `earnvouchertable` a, `vouchertable` b WHERE a.`memberID` = ? AND DATE_FORMAT(NOW(), '%Y-%m-%d') >= DATE_FORMAT(a.`startDate`, '%Y-%m-%d') AND DATE_FORMAT(date_add(a.`endDate`, INTERVAL 3 MONTH), '%Y-%m-%d') >= DATE_FORMAT(NOW(), '%Y-%m-%d') AND a.`voucherID` = b.`voucherID` AND a.`status` = 'active' AND b.`status` = 'active' AND a.`type` != 'birthday' AND b.`voucherID` NOT IN (SELECT `voucherID` FROM `vouchertable` WHERE `status` = 'active' AND `type` = 'wemissyou' AND (SELECT DATEDIFF(NOW(), @v_lastTransaction)) < CAST(`wmyperiod` AS UNSIGNED)) GROUP BY b.`voucherID`";

						} else {
							$qry = "SELECT a.*, b.`description`, b.`terms`, COUNT(*) AS `count` FROM `earnvouchertable` a, `vouchertable` b WHERE a.`memberID` = ? AND DATE_FORMAT(NOW(), '%Y-%m-%d') >= DATE_FORMAT(a.`startDate`, '%Y-%m-%d') AND DATE_FORMAT(date_add(a.`endDate`, INTERVAL 3 MONTH), '%Y-%m-%d') >= DATE_FORMAT(NOW(), '%Y-%m-%d') AND a.`voucherID` = b.`voucherID` AND a.`status` = 'active' AND b.`status` = 'active' AND a.`type` NOT IN ('birthday', 'registration') AND b.`voucherID` NOT IN (SELECT `voucherID` FROM `vouchertable` WHERE `status` = 'active' AND `type` = 'wemissyou' AND (SELECT DATEDIFF(NOW(), @v_lastTransaction)) < CAST(`wmyperiod` AS UNSIGNED)) GROUP BY b.`voucherID`";
						}
					}
			...
			$output = process_qry($qry, array($data->memberID), $logs);
			...
			$logs->write_logs("Success - My Voucher List", $file_name, array(array("response"=>"Success", "data"=>array($param))));

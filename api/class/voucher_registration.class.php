...
class registration
{
	...
	public function get_registration($param) {
		global $logs;
				...
				$updateVoucherRegistrationStatus = process_qry("UPDATE `vouchertable` a SET `status` = 'expired' WHERE `status` = 'active' AND `type` = 'registration' AND DATE_FORMAT(NOW(), '%Y-%m-%d') >= DATE_FORMAT(a.`startDate`, '%Y-%m-%d') AND DATE_FORMAT(NOW(), '%Y-%m-%d') >= DATE_FORMAT(a.`endDate`, '%Y-%m-%d')  ", array());
				

				$updateEarnVoucherRegistration = process_qry("UPDATE `earnvouchertable` set status ='expired' WHERE status =  (SELECT status FROM vouchertable a WHERE DATE_FORMAT(NOW(), '%Y-%m-%d') >= DATE_FORMAT(a.`startDate`, '%Y-%m-%d') AND DATE_FORMAT(a.`endDate`, '%Y-%m-%d') >= DATE_FORMAT(NOW(), '%Y-%m-%d') AND  a.`status` = 'expired' and a.`type` = 'registration' AND `memberID` = ? )", array($data->memberID),$logs);
